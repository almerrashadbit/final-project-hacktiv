export function doctorScheduleObject(doctorSelect) {
    ...
    doctorSelect.schedule.split(',').forEach(schedule => {
        ...
        const timeSchedule = timeRange.trim().split(' - ').map(timeElement => timeElement[0]);
        for(let i = daySelect[daySchedule[0]]; i <= daySelect[daySchedule[daySchedule.length - 1]]; i++){
          allDaySchedule[allDaySchedule.length] = {
            daySchecule: dayKey[i],
